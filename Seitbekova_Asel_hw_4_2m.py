...
class Witcher(Hero):
    def __init__(self, name, health, damage):
        Hero.__init__(self, name, health, damage, "GIVE LIVE")

    def apply_super_power(self, boss, heroes):
        for h in heroes:
            if h.health == 0:
                print(f'{self.name} give live for {h.name}')
                break
...
class Thore(Hero):
    def __init__(self, name, health, damage):
        Hero.__init__(self, name, health, damage, "SUPER HIT")
